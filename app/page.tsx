...
const guideSteps = [
  ...
  },
  {
    ...
    description: "See transcript, timing, summary, and translation cards generated by the system.",
  },
  {
...
export default function Home() {
  ...
  const translation = result?.pipeline && typeof result.pipeline === "object"
    ...
    : null;
  const timings = result?.pipeline && typeof result.pipeline === "object"
    ? (result.pipeline as Record<string, unknown>).timings
    : null;
  ...
  const timingEntries = timings && typeof timings === "object"
    ? Object.entries(timings as Record<string, unknown>)
        ...
        .map(([label, value]) => ({
          ...
          value: typeof value === "number" ? `${Math.round(value)} ms` : String(value),
        }))
    : [];
      ...
      <section className="workflow-section" aria-label="Interactive workflow and input panel">
        ...
        <div className="workflow-content">
          ...
          <section className="workflow-results" aria-label="System output">
            ...
            <article className="result-card timing-card">
              <h2>Timings</h2>
              {timingEntries.length > 0 ? (
                <dl className="timing-list">
                  {timingEntries.map((entry) => (
                    <div key={entry.label} className="timing-item">
                      <dt>{entry.label}</dt>
                      <dd>{entry.value}</dd>
                    </div>
                  ))}
                ...
                <p>Timing data appears after processing.</p>
              )}
            </article>
